<div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    ...
    <button onclick="openAddModal()" class="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-sm flex items-center">
...
<div id="addModal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <form id="modalForm" action="<?= BASEURL; ?>/admin/jadwal_jumat_add" method="POST">
                ...
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="date" name="tanggal" id="modal_tanggal" class="block w-full border-gray-300 shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm py-2" required>
                        </div>
                        
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Jam Sholat</label>
                            <input type="time" name="waktu" id="modal_waktu" value="11:45" class="block w-full border-gray-300 shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm py-2" required>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Khatib</label>
                            <input type="text" name="khatib" id="modal_khatib" class="block w-full border-gray-300 shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm py-2" placeholder="Masukkan Nama Khatib">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Imam</label>
                            <input type="text" name="imam" id="modal_imam" class="block w-full border-gray-300 shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm py-2" placeholder="Masukkan Nama Imam">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Muadzin</label>
                            <input type="text" name="muadzin" id="modal_muadzin" class="block w-full border-gray-300 shadow-sm focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm py-2" placeholder="Masukkan Nama Muadzin">
                        </div>
                    </div>
                </div>
...
<script>
    function openAddModal() {
        ...
        document.getElementById('modalForm').action = "<?= BASEURL; ?>/admin/jadwal_jumat_add";
        document.getElementById('modal-title').innerText = "Tambah Jadwal Jumat";
    ...
    function editJadwal(data) {
        document.getElementById('modalForm').reset();
        document.getElementById('modalForm').action = "<?= BASEURL; ?>/admin/jadwal_jumat_edit/" + data.id;
        document.getElementById('modal-title').innerText = "Edit Jadwal Jumat";
        document.getElementById('modal_id').value = data.id;
        document.getElementById('modal_tanggal').value = data.tanggal;
        document.getElementById('modal_waktu').value = data.waktu;
        document.getElementById('modal_khatib').value = data.khatib;
        document.getElementById('modal_imam').value = data.imam;
        document.getElementById('modal_muadzin').value = data.muadzin;
        document.getElementById('addModal').classList.remove('hidden');
    }
...
<div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
    ...
    <div class="p-4 border-b border-gray-200 flex flex-col sm:flex-row justify-between items-center bg-gray-50/50 gap-4">
        <div class="relative max-w-sm w-full">
            ...
            <input type="text" class="pl-10 block w-full border-gray-300 sm:text-sm focus:ring-emerald-500 focus:border-emerald-500 py-2" placeholder="Cari jadwal...">
        </div>
    </div>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex justify-end space-x-2">
                                <button onclick='editJadwal(<?= json_encode($row); ?>)' class="text-gray-400 hover:text-indigo-600 transition-colors" title="Edit">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </button>
                                <button type="button" onclick="openDeleteModal('<?= BASEURL; ?>/admin/jadwal_jumat_delete/<?= $row['id']; ?>')" class="text-gray-400 hover:text-red-600 transition-colors" title="Hapus">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
