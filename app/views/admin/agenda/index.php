<div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
    <div>
        ...
        <p class="text-gray-600">Kelola agenda kegiatan masjid.</p>
    </div>
    <button onclick="openAgendaAddModal()" class="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-lg text-sm font-medium shadow-sm flex items-center">
...
<div id="agendaModal" class="fixed inset-0 z-50 hidden overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
        <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" aria-hidden="true" onclick="closeAgendaModal()"></div>

        <span class="hidden sm:inline-block sm:align-middle sm:h-screen" aria-hidden="true">&#8203;</span>

        <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            <form id="agendaModalForm" action="<?= BASEURL; ?>/admin/agenda_add" method="POST">
                <input type="hidden" name="id" id="agenda_modal_id">
                <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                    <div class="flex justify-between items-center mb-5">
                        <h3 class="text-lg leading-6 font-medium text-gray-900" id="agenda-modal-title">
                            Tambah Agenda
                        </h3>
                        <button type="button" class="text-gray-400 hover:text-gray-500" onclick="closeAgendaModal()">
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                    ...
                    <button type="button" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-emerald-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm" onclick="closeAgendaModal()">
                        Batal
                    </button>
                </div>
            </form>
...
<script>
    function openAgendaAddModal() {
        document.getElementById('agendaModalForm').reset();
        document.getElementById('agendaModalForm').action = "<?= BASEURL; ?>/admin/agenda_add";
        document.getElementById('agenda-modal-title').innerText = "Tambah Agenda";
        document.getElementById('agenda_modal_id').value = "";
        document.getElementById('agendaModal').classList.remove('hidden');
    }

    function editAgenda(data) {
        document.getElementById('agendaModalForm').reset();
        document.getElementById('agendaModalForm').action = "<?= BASEURL; ?>/admin/agenda_edit/" + data.id;
        document.getElementById('agenda-modal-title').innerText = "Edit Agenda";
        document.getElementById('agenda_modal_id').value = data.id;
        document.getElementById('agenda_modal_nama').value = data.nama_kegiatan;
        document.getElementById('agenda_modal_tanggal').value = data.tanggal;
        document.getElementById('agenda_modal_waktu').value = data.waktu;
        document.getElementById('agenda_modal_deskripsi').value = data.deskripsi;
        document.getElementById('agendaModal').classList.remove('hidden');
    }
                    ...
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex justify-end space-x-2">
                                <button onclick='editAgenda(<?= json_encode($row); ?>)' class="text-gray-400 hover:text-indigo-600 transition-colors" title="Edit">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
                                    </svg>
                                </button>
                                <button type="button" onclick="openDeleteModal('<?= BASEURL; ?>/admin/agenda_delete/<?= $row['id']; ?>')" class="text-gray-400 hover:text-red-600 transition-colors" title="Hapus">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
